--PROJECT ON LIBRARY MANAGMENT SYSTEM
use library;
...
--CREATING TABLE OF ISSUED_STATUS--
...
--CREATING TABLE OF RETURN_STATUS--
...
INSERT INTO books (isbn, book_title, category, rental_price, status, author, publisher)
VALUES 
('978-1-60129-456-2', 'To Kill a Mockingbird', 'Classic', 6.00, 1, 'Harper Lee', 'J.B. Lippincott & Co.');
...
select e1.* ,b.manager_id,e2.emp_name from employee as e1 
join Branch as  b 
...
select * from books;
--lets assume the threshold rental price is >7 
select * from books
...
--issued status == members table == books == return status 
...
-- Write a query to update the status of books in the books table to "Yes" when they 
-- are returned (based on entries in the return_status table).
...
-- )
-- BEGIN
--     -- Insert return details into the return_records table
--     INSERT INTO return_records (return_id, issue_id, book_quality)
--     VALUES (p_return_id, p_issue_id, p_book_quality);
-- END$$

-- DELIMITER ;

-- CALL add_return_records(101, 2001, 'Good');
